...
oncologist = AssistantAgent(
    ...
    system_message="""You are a medical oncologist and the lead of this tumor board.
    Your role is to drive the case review by asking each specialist for their input, one by one.
    ...
    4.  After gathering all information, create a concise case summary and then request a surgical evaluation from the Surgeon.
    5.  After the Surgeon provides their evaluation, ask the team if they have any additional comments or recommendations based on the complete picture.
    6.  Finally, synthesize all findings and the discussion into a final, consolidated treatment plan.
...
ehr_analyst = AssistantAgent(
    ...
    model_client=model_client,
    system_message="You are an EHR Analyst. When asked, provide the patient summary using your tool.",
)
...
radiologist = AssistantAgent(
    ...
    model_client=model_client,
    system_message="You are a radiologist. When asked, provide the imaging findings using your tool.",
)
...
pathologist = AssistantAgent(
    ...
    model_client=model_client,
    system_message="You are a pathologist. When asked, provide the pathology diagnosis using your tool.",
)
...
surgeon = AssistantAgent(
    ...
    model_client=model_client,
    system_message="You are a surgeon. When the Oncologist provides a case summary and asks for your evaluation, provide your reasoning based on your knowledge.",
)
...
def custom_speaker_selection(messages: Sequence[BaseChatMessage]) -> str:
    ...
    if last_message.source == "Oncologist":
        # Use regex to find the agent name that is being addressed, typically followed by a comma.
        # This is more robust than a simple "in" check.
        match = re.search(r"\b(EHR_Analyst|Radiologist|Pathologist|Surgeon)\b,", last_message.content)
        if match:
...
team = SelectorGroupChat(
    participants=[oncologist, ehr_analyst, radiologist, pathologist, surgeon],
